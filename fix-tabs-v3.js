...
// Determine tab boundaries
for (let i = 0; i < tabKeys.length; i++) {
  tabKeys[i].start = tabKeys[i].keyLine - 1;
  if (i < tabKeys.length - 1) {
    tabKeys[i].end = tabKeys[i + 1].keyLine - 2;
  } else {
    tabKeys[i].end = tabItemsEndIdx - 1;
  }
}
...
// Helper: find outer Card inner content using depth counting
function extractCardInner(tabLines) {
  let cardOpenIdx = -1;
  let depth = 0;
  let cardCloseIdx = -1;
  ...
  if (cardOpenIdx === -1 || cardCloseIdx === -1) {
    console.error('Could not find Card boundaries');
    console.error('cardOpenIdx:', cardOpenIdx, 'cardCloseIdx:', cardCloseIdx);
    process.exit(1);
  }
  
  return tabLines.slice(cardOpenIdx + 1, cardCloseIdx);
}
...
const salesInner = extractCardInner(tabContents.sales);
console.log('sales inner lines:', salesInner.length);

// 3. Extract inner content of settlements Card
const settlementsInner = extractCardInner(tabContents.settlements);
...
// 6. Rebuild file
const prefix = lines.slice(0, tabItemsStartIdx + 1);
const suffix = lines.slice(tabItemsEndIdx);
...
// 7. Fix useEffect - use exact match from current file
...
if (newContent.includes(oldEffect)) {
  ...
  // Try to log a snippet for debugging
  const effectIdx = newContent.indexOf('useEffect(() => {');
  if (effectIdx !== -1) {
    console.log('Found useEffect at index', effectIdx);
    console.log('Snippet:', JSON.stringify(newContent.substring(effectIdx, effectIdx + 200)));
  }
}
